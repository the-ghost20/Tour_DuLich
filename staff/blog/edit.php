  ...
  if ($row && $_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($title === '') {
          ...
          try {
              $pdo->prepare(
                  'UPDATE blog_posts SET title=:t, slug=:s, excerpt=:e, featured_image=:fi, category=:cat, tag_label=:tl, keywords=:kw, body=:b, status=:st, published_at=:p WHERE id=:id'
              )->execute([
                  't'    => $title,
                  's'    => $slug,
                  'e'    => $ex === '' ? null : $ex,
  ...
  $pageSubtitle = 'Cập nhật nội dung đã đăng';
  $activePage   = 'blog';
    ...
    <div class="data-card" style="max-width:720px">
      ...
      <form method="post" style="display:grid;gap:12px;padding:8px 4px 16px">
        ...
        <div>
          ...
          <input class="form-control" name="featured_image" value="<?= h((string) ($_POST['featured_image'] ?? (string) ($row['featured_image'] ?? ''))) ?>" />
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            <label class="cell-muted" style="font-size:0.8rem">Chuyên mục</label>
            <?php $pcat = (string) ($_POST['category'] ?? ($row['category'] ?? 'cam-nang')); ?>
            <select class="form-control" name="category">
              ...
              <option value="am-thuc" <?= $pcat === 'am-thuc' ? 'selected' : '' ?>>Ẩm thực</option>
              <option value="tin-tuc" <?= $pcat === 'tin-tuc' ? 'selected' : '' ?>>Tin & KM</option>
          ...
          <div>
            ...
            <input class="form-control" name="tag_label" value="<?= h((string) ($_POST['tag_label'] ?? (string) ($row['tag_label'] ?? ''))) ?>" />
          </div>
        </div>
        <div>
          <label class="cell-muted" style="font-size:0.8rem">Từ khóa</label>
          <input class="form-control" name="keywords" value="<?= h((string) ($_POST['keywords'] ?? (string) ($row['keywords'] ?? ''))) ?>" />
        </div>
        <div>
