...
function initWishlistPage() {
  console.log('initWishlistPage called');
  const root = document.getElementById("wishlist-page-root");
  const box = document.getElementById("wishlist-page-list");
  console.log('root:', root, 'box:', box);
  if (!root || !box) {
    console.log('root or box not found');
    return;
  }

  // Re-render when page gains focus (e.g., switching tabs)
  window.addEventListener('focus', () => {
    render();
  });

  // Also listen for storage changes (if localStorage updated in another tab)
  ...
  function render() {
    ...
    const actionsDiv = document.getElementById("wishlist-page-actions");
    ...
    if (list.length === 0) {
      ...
      if (actionsDiv) actionsDiv.innerHTML = '<button type="button" id="clear-all-wishlist" class="btn btn-secondary">Xóa tất cả</button>';
      return;
    }
    ...
    if (clearAllBtn && !clearAllBtn.dataset.boundClearAll) {
      clearAllBtn.addEventListener("click", () => {
        ...
        if (currentList.length === 0) {
          alert("Không có tour yêu thích nào để xóa.");
          return;
        }
        if (confirm("Bạn có chắc muốn xóa tất cả tour yêu thích?")) {
          ...
          loadWishlistUI();
          render();
          showNotification("Đã xóa tất cả tour yêu thích", "info");
        }
      });
      clearAllBtn.dataset.boundClearAll = "1";
      console.log('Event bound to clear all button');
    }
  }
